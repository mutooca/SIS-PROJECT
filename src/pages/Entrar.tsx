...
const userSchemaLogin = z.object({
  email: z.string().min(1, 'O e-mail é obrigatório').transform(sanitizeEmail).refine(val => val.length > 0, 'O e-mail não pode estar vazio').pipe(
      z.string().email('Insira um e-mail válido').max(254, 'E-mail demasiado longo').regex(/^[a-z0-9._+-]+@[a-z0-9.-]+\.[a-z]{2,}$/, 'Formato de e-mail inválido')),
  
  password: z.string().min(1, 'A palavra-passe é obrigatória').min(8, 'A palavra-passe deve ter no mínimo 8 caracteres').max(64, 'Palavra-passe demasiado longa')
});
...
export default function Entrar(){
  ...
  async function handleLogin(data: UserLoginData){
    try {
      ...
      console.log('Tipo de login:', role)
      await new Promise(resolve => setTimeout(resolve, 1000))
      alert(`Login ${role} realizado com sucesso!`)
    } catch (error) {
      ...
      <div className="justify-center rounded-xl bg-white py-8 shadow flex items-center flex-col border h-full max-w-lg m-auto space-y-3">
        <div className="space-y-3 w-full px-10">
          ...
          <ul className="grid grid-cols-3 gap-2 bg-indigo-50 py-1 px-2 rounded-lg">
            <button 
              onClick={() => handleSubmitRole('utente')} 
              className={`hover:bg-gray-50 transition py-1 rounded-lg text-center font-semibold ${role === 'utente' ? 'bg-gray-50': ''}`}>Utente</button>
        ...
        {role === 'utente' && (
          <form onSubmit={handleSubmit(handleLogin)} className="space-y-3 w-full justify-center flex items-center flex-col px-10">
            ...
            <button 
              type="submit" 
              disabled={isSubmitting}
              className="bg-blue-500 text-white hover:bg-blue-400 disabled:bg-gray-400 disabled:cursor-not-allowed w-full h-10 rounded-xl"
            >
        ...
        {role === 'clinico' && (
          <form onSubmit={handleSubmit(handleLogin)} className="space-y-3 w-full justify-center flex items-center flex-col px-10">
            ...
            <div className="space-y-1 flex w-full flex-col max-w-full">
              <label htmlFor="email-clinico" className="font-semibold">E-mail *</label>
              <input 
                type="email" 
                {...register('email')} 
                id="email-clinico" 
            ...
            <div className="space-y-1 flex flex-col w-full">
              <label htmlFor="password-clinico" className="font-semibold">Palavra-passe *</label>
              <input 
                {...register('password')} 
                type="password" 
                id="password-clinico" 
        ...
        {role === 'admin' && (
          <form onSubmit={handleSubmit(handleLogin)} className="space-y-3 w-full justify-center flex items-center flex-col px-10">
            ...
            <div className="space-y-1 flex w-full flex-col max-w-full">
              <label htmlFor="email-admin" className="font-semibold">E-mail *</label>
              <input 
                type="email" 
                {...register('email')} 
                id="email-admin" 
            ...
            <div className="space-y-1 flex flex-col w-full">
              <label htmlFor="password-admin" className="font-semibold">Palavra-passe *</label>
              <input 
                {...register('password')} 
                type="password" 
                id="password-admin" 
